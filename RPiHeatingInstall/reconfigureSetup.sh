#!/bin/bash
cd /home/pi
echo -e "\x1b[0;32mINSTALLER:\x1b[m  Reconfigure Options for Collie147s Raspberry Pi Heating System"
echo
while ps ax | grep -v grep | grep python > /dev/null; do
	sudo pkill python
	sleep 1
done
...
select opt in $OPTIONS2; do
	if [ "$opt" = "433mhz" ]; then
	  ...
	  echo -e "\x1b[0;32mINSTALLER:\x1b[0;33m                 You will need to configure this!\x1b[m"
	  SUBOPTION3="Yes No"
...
select opt in $OPTION6; do
	if [ "$opt" = "Reboot" ]; then
		...
		sudo python /home/pi/RPiHeatingScreen.py
		break
	elif [ "$opt" = "Exit" ]; then
		break
		else
		clear
		echo -e "\x1b[0;32mINSTALLER:\x1b[m  bad option enter 1, 2 or 3"
		echo -e "\x1b[0;32mINSTALLER:\x1b[m  1. Reboot"
		echo -e "\x1b[0;32mINSTALLER:\x1b[m  2. Restart"
		echo -e "\x1b[0;32mINSTALLER:\x1b[m  3. Exit"
	fi
done
exit
